    ...
    benefits_list = [
        {
            'id': 1,
            'type': 'Pet Insurance',
            'details': 'Comprehensive coverage for your furry family members including veterinary care, medications, and emergency services.',
            'status': 'not_enrolled',
        ...
        },
        {
            ...
            'details': 'Full medical, dental, and vision coverage for you and your family.',
            'status': 'enrolled',
...
@app.route('/flexible-work')
def flexible_work():
    return render_template('flexible-work.html')
...
groups = [
    ...
    },
    {
        ...
        'members': ['Emily Davis', 'Tom Brown', 'Lisa Garcia'],
    ...
    },
    {
        'id': 3,
        'name': 'Innovation Lab',
        'description': 'R&D team exploring new technologies',
        'members': ['David Lee', 'Rachel Kim', 'Chris Wong'],
...
projects = [
    ...
    },
    {
        ...
        'group_id': 2,
        'description': 'Spring marketing campaign for new product launch',
        'status': 'planning',
        'progress': 25,
        'due_date': '2024-04-01',
        'assigned_to': ['Emily Davis', 'Tom Brown']
    },
    {
        'id': 3,
        'name': 'AI Integration Research',
        'group_id': 3,
        'description': 'Research and implementation of AI features',
        'status': 'completed',
        'progress': 100,
        'due_date': '2024-01-25',
        'assigned_to': ['David Lee', 'Rachel Kim']
    }
]

# Global applications data
applications = [
    {
        'id': 1,
        ...
        'cover_letter': 'john_doe_cover.pdf',
        'status': 'pending',
    ...
    },
    {
        ...
        'cover_letter': 'jane_smith_cover.pdf',
        'status': 'reviewed',
        'applied_date': '2024-01-12',
        'experience_years': 5,
        'skills': ['Digital Marketing', 'SEO', 'Google Ads'],
        'education': 'MBA in Marketing',
        'current_company': 'Marketing Solutions',
        'expected_salary': '$75,000',
        'availability': '2 weeks notice',
        'notes': 'Excellent track record in digital marketing campaigns'
    },
    {
        'id': 3,
        ...
        'cover_letter': 'mike_johnson_cover.pdf',
        'status': 'approved',
        'applied_date': '2024-01-10',
        'experience_years': 2,
        'skills': ['SQL', 'Python', 'Tableau'],
        'education': 'Master in Data Science',
        'current_company': 'Data Insights Inc',
        'expected_salary': '$65,000',
        'availability': 'Immediate',
        'notes': 'Strong analytical skills, good with data visualization'
    }
]

# Global job applications data
job_applications = [
    {
        'id': 1,
        'job_id': 1,
    ...
    },
    {
        ...
        'email': 'jane.smith@example.com',
        'status': 'reviewed',
        'applied_at': '2024-01-14 14:20:00',
        'resume_score': 92,
        'experience_match': 95,
        'skills_match': 90
    },
    {
        ...
        'email': 'mike.johnson@example.com',
        'status': 'approved',
        'applied_at': '2024-01-13 09:15:00',
        'resume_score': 78,
        'experience_match': 85,
        'skills_match': 82
    }
...
# Dummy data stores
mock_user = {"email": "user@example.com", "password": "password"}
mock_admin = {"email": "admin@example.com", "password": "password"}
...
def register_user():
    if request.method == "POST":
        ...
        # Create new user
        user_id = db_helper.create_user(email, password)
        if user_id:
...
def login():
    if request.method == "POST":
        try:
            ...
            # For testing purposes, if the user doesn't exist, create default users
            if not user:
                print("User not found, creating default users...")  # Debug log
                default_users = [
                ...
                for default_email, role in default_users:
                    db_helper.create_user(default_email, "password", role)
                
                # Try getting the user again
                user = db_helper.get_user_by_email(email)
                print(f"After creation, user: {user}")  # Debug log
            
            if user and password == "password":  # For testing, accept plain "password"
                session.clear()
                session["user"] = email
...
def admin_badges():
    ...
    # Sample badges data
    badges = [
        {'id': 1, 'name': 'Python Expert', 'description': 'Advanced Python programming skills', 'icon': '🐍', 'category': 'Technical'},
        {'id': 2, 'name': 'Team Player', 'description': 'Excellent collaboration skills', 'icon': '🤝', 'category': 'Soft Skills'},
        {'id': 3, 'name': 'Problem Solver', 'description': 'Strong analytical thinking', 'icon': '🧠', 'category': 'Problem Solving'},
        {'id': 4, 'name': 'Leadership', 'description': 'Demonstrated leadership abilities', 'icon': '👑', 'category': 'Leadership'},
        {'id': 5, 'name': 'Communication Pro', 'description': 'Excellent communication skills', 'icon': '🗣️', 'category': 'Communication'},
        {'id': 6, 'name': 'Innovation', 'description': 'Creative problem solving', 'icon': '💡', 'category': 'Innovation'}
    ]
...
def admin_employees():
    ...
    return render_template('admin_employees.html', employees=users)
...
def employee_leave():
    ...
    if request.method == 'POST':
        ...
        if leave_type and start_date and end_date and reason:
            ...
            # Add notification for admin
            add_notification("admin@example.com", f"New leave request from {session['user']}: {leave_type} from {start_date} to {end_date}", "warning")
...
def employee_career_development():
    ...
    knowledge_docs = knowledge_documents[:5]  # Show recent documents
    
    return render_template("employee_portal/career_development.html", 
...
def employee_support():
    ...
    return render_template("employee_portal/support.html", support_options=support_options)
...
def employee_groups():
    ...
    return render_template('employee_portal/groups.html', 
                         groups=groups, 
                         chat_messages=chat_messages, 
                         projects=projects)

@app.route('/employee/groups/<int:group_id>/add-member', methods=['POST'])
    ...
    for group in groups:
        if group['id'] == group_id:
            if member_name not in group['members']:
                ...
                return jsonify({
                    "success": True,
                    "message": f"{member_name} added to {group['name']}"
                })
            else:
                return jsonify({"error": "Member already exists in group"}), 400
    
    return jsonify({"error": "Group not found"}), 404

@app.route('/employee/groups/<int:group_id>/upload-file', methods=['POST'])
def upload_group_file(group_id):
    ...
    if file and allowed_file(file.filename):
        ...
        user_email = session['user']
        sender_name = user_profiles.get(user_email, {}).get('name', user_email)
...
def admin_review_application(application_id):
    ...
    for app in applications:
        if app['id'] == application_id:
            app['status'] = 'reviewed'
            flash('Application marked as reviewed', 'success')
            break
...
@app.route('/employee/groups/<int:group_id>/send-message', methods=['POST'])
def send_group_message(group_id):
    ...
    # Add to chat messages (in a real app, this would go to a database)
    chat_messages.append(new_message)
...
def add_device():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_asset_management'))
    
    return render_template('admin/add_device.html')

@app.route('/devices/assign', methods=['GET', 'POST'])
def assign_device():
    if "user" not in session or session.get('role') != 'admin':
        return redirect(url_for("login"))
    ...
    if request.method == 'POST':
        ...
        for device in devices:
            if device['id'] == device_id:
                device['assigned_to'] = employee_email
    ...
    # Get available devices and employees
    available_devices = [d for d in devices if d['status'] == 'available']
    employees = list(user_profiles.keys())
    
    return render_template('admin/assign_device.html', 
                         devices=available_devices, 
                         employees=employees)

# Global chat messages for groups
chat_messages = [
    {
        'id': 1,
        'group_id': 1,
